...
type TSCE = class
  private
   ...
   public
    ...
    property TOK : TStringList read FTOK write FTOK;
    ...
    function LoadSCE(Filename: String):boolean;
    procedure SaveSCE(Filename: String; WidthDebugInfo: Boolean = false);
...
constructor TSCE.Create();
begin
    FDEF := TStringList.Create();
    FCTL := TStringList.Create();
    FTOK := TStringList.Create();
...
function TSCE.LoadSCE(Filename: String):boolean;
...
procedure TSCE.SaveSCE(Filename: String; WidthDebugInfo: Boolean = false);
...
function TSCE.AddBlock(NameTag : String; Content: TStringList; Header: String):boolean;
var StrAux : String;
    ProcNum :   integer;
...
begin
 ...
 if (NameTag = 'TOK') then FTOK.Text := Content.Text else
...
procedure TSCE.SetProcessCode(i: longint; Value: String);
begin
  if (not  Assigned(FProcesses[i])) then FProcesses[i] := TStringList.Create();
  FProcesses[i].Text:= Value;
end;

function TSCE.GetProcess(i:longint) : TStringList;
begin
...
function TSCE.AddProcess():boolean;
var num : integer;
begin
 num := FLastProcess + 1;
 if (num <256) then
...
function TSCE.SetInterruptProcess(ProcessID: Byte):boolean;
begin
 if (ProcessID > 2) then
 begin
...
function TSCE.getIdentifierList():TStringList;
